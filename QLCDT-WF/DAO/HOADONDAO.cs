        ...
        public static void sendmail()
        {
            ...
            foreach(var data in dskhchuathanhtoan)
            {
                try
                {
                    ...
                    mailclient.Credentials = new NetworkCredential("boykunis90@gmail.com","memaj123");

                    MailMessage message = new MailMessage("boykunis90@gmail.com", "boykunis90@gmail.com");
                ...
                {
                    MessageBox.Show("Mail gửi thất bại !!!" +ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
        }
        ...
        public static void tongketthanghientai()
        {
            ...
            if(ktractsd != null)
            {
                foreach(var data in ktractsd)
                {
                    ...
                    DateTime tgbd = new DateTime(2000, 12, 25, data.TGBD.Hour, data.TGBD.Minute, data.TGBD.Second);

                    string bd = string.Format("{0:HH:mm:ss}", tgbd);

                    DateTime tgkt = new DateTime(2000, 12, 25, data.TGKT.Hour, data.TGKT.Minute, data.TGKT.Second);
                    ...
                    HOADON update = (from a in db.HOADONs
                                         where a.IDSIM == data.IDSIM && a.THANG == data.TGBD.Month && a.NAM == data.TGBD.Year
                                         select a).Single();
                        update.TONGTIEN = update.TONGTIEN + CTSDDAO.tinhphi(bd,kt);
                        db.SaveChanges();
